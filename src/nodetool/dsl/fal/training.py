...
class HunyuanVideoLoraTraining(SingleOutputGraphNode[dict[str, Any]], GraphNode[dict[str, Any]]):
    ...
    images_data_url: types.ImageRef | OutputHandle[types.ImageRef] = connect_field(default=types.ImageRef(type='image', uri='', asset_id=None, data=None, metadata=None), description='URL to zip archive with images. Try to use at least 4 images in general the more the better. In addition to images the archive can contain text files with captions. Each text file should have the same name as the image file it corresponds to.')
...
class QwenImageEditTrainer(SingleOutputGraphNode[dict[str, Any]], GraphNode[dict[str, Any]]):
    ...
    steps: int | OutputHandle[int] = connect_field(default=1000, description='Number of steps to train for')
    image_data_url: types.ImageRef | OutputHandle[types.ImageRef] = connect_field(default=types.ImageRef(type='image', uri='', asset_id=None, data=None, metadata=None), description='URL to the input data zip archive. The zip should contain pairs of images. The images should be named: ROOT_start.EXT and ROOT_end.EXT For example: photo_start.jpg and photo_end.jpg The zip can also contain a text file for each image pair. The text file should be named: ROOT.txt For example: photo.txt This text file can be used to specify the edit instructions for the image pair. If no text file is provided, the default_caption will be used. If no default_caption is provided, the training will fail.')
    learning_rate: float | OutputHandle[float] = connect_field(default=0.0001, description='Learning rate for LoRA parameters.')
    default_caption: str | OutputHandle[str] = connect_field(default='', description='Default caption to use when caption files are missing. If None, missing captions will cause an error.')
...
class QwenImageTrainer(SingleOutputGraphNode[dict[str, Any]], GraphNode[dict[str, Any]]):
    ...
    image_data_url: types.ImageRef | OutputHandle[types.ImageRef] = connect_field(default=types.ImageRef(type='image', uri='', asset_id=None, data=None, metadata=None), description='URL to zip archive with images for training. The archive should contain images and corresponding text files with captions. Each text file should have the same name as the image file it corresponds to (e.g., image1.jpg and image1.txt). If text files are missing for some images, you can provide a trigger_phrase to automatically create them. Supported image formats: PNG, JPG, JPEG, WEBP. Try to use at least 10 images, although more is better.')
...
class RecraftV3CreateStyle(SingleOutputGraphNode[Any], GraphNode[Any]):
    ...
    images_data_url: types.ImageRef | OutputHandle[types.ImageRef] = connect_field(default=types.ImageRef(type='image', uri='', asset_id=None, data=None, metadata=None), description='URL to zip archive with images, use PNG format. Maximum 5 images are allowed.')
...
class TurboFluxTrainer(SingleOutputGraphNode[dict[str, Any]], GraphNode[dict[str, Any]]):
    ...
    images_data_url: types.ImageRef | OutputHandle[types.ImageRef] = connect_field(default=types.ImageRef(type='image', uri='', asset_id=None, data=None, metadata=None), description='URL to zip archive with images of a consistent style. Try to use at least 10 images, although more is better.')
