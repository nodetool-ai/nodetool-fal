...
class FfmpegApiLoudnorm(SingleOutputGraphNode[types.AudioRef], GraphNode[types.AudioRef]):
    """
    ...
    audio: types.AudioRef | OutputHandle[types.AudioRef] = connect_field(default=types.AudioRef(type='audio', uri='', asset_id=None, data=None, metadata=None), description='URL of the audio file to normalize')
    integrated_loudness: float | OutputHandle[float] = connect_field(default=-18, description='Integrated loudness target in LUFS.')
    loudness_range: float | OutputHandle[float] = connect_field(default=7, description='Loudness range target in LU')
...
class FfmpegApiMetadata(SingleOutputGraphNode[Any], GraphNode[Any]):
    ...
    extract_frames: bool | OutputHandle[bool] = connect_field(default=False, description='Whether to extract the start and end frames for videos. Note that when true the request will be slower.')
    media: types.VideoRef | OutputHandle[types.VideoRef] = connect_field(default=types.VideoRef(type='video', uri='', asset_id=None, data=None, metadata=None, duration=None, format=None), description='URL of the media file (video or audio) to analyze')
...
class FfmpegApiWaveform(SingleOutputGraphNode[dict[str, Any]], GraphNode[dict[str, Any]]):
    """
    ...
    precision: int | OutputHandle[int] = connect_field(default=2, description='Number of decimal places for the waveform values. Higher values provide more precision but increase payload size.')
    smoothing_window: int | OutputHandle[int] = connect_field(default=3, description='Size of the smoothing window. Higher values create a smoother waveform. Must be an odd number.')
    media: types.AudioRef | OutputHandle[types.AudioRef] = connect_field(default=types.AudioRef(type='audio', uri='', asset_id=None, data=None, metadata=None), description='URL of the audio file to analyze')
    points_per_second: float | OutputHandle[float] = connect_field(default=4, description='Controls how many points are sampled per second of audio. Lower values (e.g. 1-2) create a coarser waveform, higher values (e.g. 4-10) create a more detailed one.')
