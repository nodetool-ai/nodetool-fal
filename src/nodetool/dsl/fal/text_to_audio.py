...
class MMAudioV2(GraphNode):
    ...
    num_steps: int | GraphNode | tuple[GraphNode, str] = Field(
    ...
    cfg_strength: float | GraphNode | tuple[GraphNode, str] = Field(
        default=4.5, description="The strength of Classifier Free Guidance"
    )
    mask_away_clip: bool | GraphNode | tuple[GraphNode, str] = Field(
...
class StableAudio(GraphNode):
    ...
    seconds_start: int | GraphNode | tuple[GraphNode, str] = Field(
        default=0, description="The start point of the audio clip to generate"
    )
    seconds_total: int | GraphNode | tuple[GraphNode, str] = Field(
    ...
    @classmethod
    def get_node_type(cls):
        return "fal.text_to_audio.StableAudio"
