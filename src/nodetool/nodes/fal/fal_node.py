...
class FALNode(BaseNode):
    ...
    async def _convert_assets_recursive(
        ...
        if isinstance(obj, AssetRef):
            ...
            return [
                await self._convert_assets_recursive(client, item, context)
                for item in obj
            ]
    ...
    async def submit_request(
        ...
        # Remove empty string values to avoid API parsing errors
        # (e.g. seed="" would fail with "unable to parse string as an integer")
        arguments = {k: v for k, v in arguments.items() if v != ""}

        # Convert AssetRef objects to FAL storage URLs
        converted_arguments = await self._convert_assets_recursive(
